<template>
	<CModal scrollable alignment="center" fullscreen="xl" size="xl" @close="close" :keyboard="false" :visible="visible">
		...
		<CModalBody v-if="this.purchase">
			...
			<CRow v-if="!isCancelled">
				<CCard class="mb-4">
					<CCardBody>
						<CRow>
							<CCol :md="12">
								<CButton @click="updatePurchase" color="danger">Cancelar Reserva</CButton>
							</CCol>
						</CRow>
					</CCardBody>
				</CCard>
...
export default {
	...
	computed: {
    isCancelled() {
      return ["cancelled_by_traveler", 'automatically_cancelled', 'cancelled_by_agency'].includes(this.purchase.status) 
    },
		statusInBlackList() {
